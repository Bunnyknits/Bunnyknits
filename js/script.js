...
function bodyLock() {
	const lockPaddingValue = window.innerWidth - document.querySelector('.wrapper').offsetWidth + 'px';   /* когда выскакивает попап, то исчезает полоса прокрутки на всей странице и весь ее контент сдвигается вправо на ширину этого скролла, при этом изображение дергается, чтобы этого не было добавляем эту функцию */
	...
	body.classList.add('_lock');   /* body добавляется класс lock (в css) */
